...
import { Consumer, Publisher, Middleware, yaml2js } from './lib';
...
const run = async () => {
  const consumer = await new Consumer(consumerOptions);
  ...
  const publisher = await new Publisher(publisherOptions);
  publisher
    .use([
