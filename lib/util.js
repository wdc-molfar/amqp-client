...
const { validate } = require('./schemas').connection;
...
const closeConnection = async (client) => {
  const conectionKey = find(keys(pool), (c) =>
    pool[c].clients.includes(client),
  );
  if (conectionKey) {
    const clientIndex = findIndex(
      pool[conectionKey].clients,
      (c) => c === client,
    );
    if (clientIndex >= 0) {
...
const deepFilter = (object, predicate, path) => {
  ...
  if (isArray(object)) {
    object.forEach((f, index) => {
      path.push(index);
      if (!isFunction(f))
        result = result.concat(deepFilter(f, predicate, path));
      path.pop();
    });
  } else {
    keys(object).forEach((k) => {
      ...
      if (!isFunction(object[k]))
        result = result.concat(deepFilter(object[k], predicate, path));
      path.pop();
    });
  }
